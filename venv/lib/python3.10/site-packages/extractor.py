...
__all__ = [
  'Extractor', 'isBinaryType', 'EXTRACTOR_ENCODING', 'DEFAULT_LIBRARIES',
...
class Keywords(Structure):
  """
    ...
    """
  _fields_ = [('keyword', c_char_p), ('keywordType', KeywordType),
              ('next', Keywords_p)]
...
class Extractor(object):
  ...
  def __init__(self,
    ...
    if defaults:
      ...
      self._libraries = dict([(library, None)
                              for library in DEFAULT_LIBRARIES])
    if use_filename:
      self.addLibrary("libextractor_filename")
    if libraries:
      self.extractors = libextractor.EXTRACTOR_loadConfigLibraries(
        self.extractors, libraries)
      self._libraries.update(
        dict([(library, None) for library in libraries.split(':')]))
    ...
    if languages:
      [
        self.addLibraryLast("libextractor_printable_%s" % language)
  ...
  def extractFromFile(self, filename):
	...
	True
        ...
        """
    self.keywords_p = libextractor.EXTRACTOR_getKeywords(
      self.extractors, filename)
  ...
  def extractFromData(self, data, size):
	...
	also be empty if no keyword was found for the file.
        ...
        """
    self.keywords_p = libextractor.EXTRACTOR_getKeywords2(
      self.extractors, data, size)
    return self._extract()
  ...
  def _extract(self):
    ...
    while True:
      keyword_type = libextractor.EXTRACTOR_getKeywordTypeAsString(
        self.keywords.keywordType).decode(EXTRACTOR_ENCODING)
  ...
  def addLibrary(self, library):
    ...
    self.extractors = libextractor.EXTRACTOR_addLibrary(
      self.extractors, library)
  ...
  def addLibraryLast(self, library):
    ...
    self.extractors = libextractor.EXTRACTOR_addLibraryLast(
      self.extractors, library)
  ...
  def removeLibrary(self, library):
    ...
    self.extractors = libextractor.EXTRACTOR_removeLibrary(
      self.extractors, library)
  ...
  libraries = property(fget=_get_libraries,
                       fset=_set_libraries,
                       fdel=removeAllLibraries,
                       doc='tuple of loaded libraries')
